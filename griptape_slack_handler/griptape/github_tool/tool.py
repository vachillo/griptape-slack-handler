...
class GitHubUserTool(BaseTool):
    ...
    @activity(
        config={
            "description": "Can be used to approve or comment on a pull request (PR) in the Github repository. The files in the pull request should be reviewed before approving.",
            "schema": Schema(
                {
                    Literal(
                        "pull_request_id",
    ...
    @activity(
        config={
            "description": "Can be used to get data about what is changing in a pull request (PR).",
            "schema": Schema(
                {
                    Literal(
                        "pull_request_id",
    ...
    def get_pull_request_data(
        ...
        try:
            ...
            if pull_request_files.totalCount > MAX_PR_FILES:
                pull_request_files = pull_request_files[:MAX_PR_FILES]
            ...
            return ListArtifact(
                [
                    ...
                    *(
                        [
                            InfoArtifact(
                                f"Max files reached, returning the first {MAX_PR_FILES}"
                            )
                        ]
                        if pull_request_files.totalCount == MAX_PR_FILES
                        else []
