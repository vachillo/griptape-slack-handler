...
def thought_payload(thought: str):
    """Gets a payload with the thought bubble."""
    return {
        "blocks": thought_blocks(thought),
        "text": "Thought...",
    }


def markdown_payload(text: str):
...
def error_payload(error: str):
    ...
    return {
        "blocks": [error_block(error)],
        "text": "Error while processing response",
    }


def action_payload(action: str):
    """Gets a payload with the action bubble."""
    return {
        "blocks": action_blocks(action),
        "text": "Action...",
    }
