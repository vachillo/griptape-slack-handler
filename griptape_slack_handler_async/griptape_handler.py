...
from .async_util import to_async_collect, to_async
...
async def try_add_to_thread(
    ...
    rulesets = await to_async_collect(
        [lambda: Ruleset(name=mentioned_user) for mentioned_user in mentioned_user_ids]
    )
    ...
    memory = await to_async(lambda: ConversationMemory())
...
async def get_rulesets(**kwargs) -> list[Ruleset]:
    return await to_async_collect(
        [lambda: Ruleset(name=value) for value in kwargs.values()]
    )
...
async def agent(
    ...
    *,
    ...
    tools = await get_tools(message, dynamic=dynamic_tools)
    ...
    output = (await to_async(agent.run, user_id, message)).output
    if isinstance(output, ErrorArtifact):
        raise ValueError(output.to_text())
    return output.to_text()


async def is_relevant_response(message: str, response: str) -> bool:
    agent = Agent(
    ...
    output = (await to_async(agent.run, message, response)).output
    if isinstance(output, ErrorArtifact):
        raise ValueError(output.to_text())
