...
class SlackEventListenerDriver(BaseEventListenerDriver):
    ...
    _slack_response: dict = field(factory=dict, init=False)

    def try_publish_event_payload_batch(self, event_payload_batch: list[dict]) -> None:
        new_text = "".join([event.get("text") for event in event_payload_batch])
        try:
            self._slack_response = self.web_client.chat_update(
                text=self._slack_response.get("text", "") + new_text,
                ts=self.ts,
                thread_ts=self.thread_ts,
                channel=self.channel,
            ).data
        except Exception:
            self._slack_response = self.web_client.chat_postMessage(
                text=new_text,
                thread_ts=self.thread_ts,
                channel=self.channel,
            ).data
            self.ts = self._slack_response["ts"]

    def try_publish_event_payload(self, event_payload: dict) -> None:
        if not event_payload.pop("publish_message", True):
            return

        if event_payload.pop("create_new_message", False):
            self.web_client.chat_postMessage(
                **event_payload, thread_ts=self.thread_ts, channel=self.channel
            )
        else:
